...
while(True):
    ...
    if ids is not None and len(ids) > 0:
        ...
        id_now = next(iter(target_ids), None)
        if id_now is None:
            continue

        # Recompute current center for the first target
        center_2d_now, _ = cv2.projectPoints(center_3d, rvecs[0], tvecs[0], CM, dist_coef)
        center_pixel_now = center_2d_now[0][0].astype(int)
        img_h, img_w = frame.shape[:2]

        grid_x_now = int(center_pixel_now[0] / img_w * grid_width)
        grid_y_now = int(center_pixel_now[1] / img_h * grid_height)

        center_grid_now = np.array([grid_y_now, grid_x_now])

        # Loop until within threshold
        while True:
            # Distance check (pixel space; adjust if using grid units)
            dist_now = np.linalg.norm(center_grid_now - np.array(end_points[id_now]))
            if dist_now <= 1.5:
                print(f"Target {id_now} reached destination!")
                break

            gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            corners, ids, rejectedImgPoints = aruco.detectMarkers(gray, aruco_dict, parameters=parameters)

            # If no markers detected, keep trying next frame
            ...
            # Rebuild grid with other markers as obstacles
            grid.fill(1)
            for i in range(len(ids)):
                if int(ids[i][0]) == id_now:
                    continue
                center_2d, _ = cv2.projectPoints(center_3d, rvecs[i], tvecs[i], CM, dist_coef)
                center_pixel = center_2d[0][0].astype(int)
                img_h, img_w = frame.shape[:2]
            ...
            center_2d_now, _ = cv2.projectPoints(center_3d, rvecs[marker_idx], tvecs[marker_idx], CM, dist_coef)
            center_pixel_now = center_2d_now[0][0].astype(int)

            # Pathfind in grid coordinates
            img_h, img_w = frame.shape[:2]
            grid_x_now = int(center_pixel_now[0] / img_w * grid_width)
            grid_y_now = int(center_pixel_now[1] / img_h * grid_height)
            start_grid_now = np.clip([grid_x_now, grid_y_now], 0, [grid_width - 1, grid_height - 1]).astype(int)
            start_pt = (int(start_grid_now[1]), int(start_grid_now[0]))  # (row, col)
            end_pt = (int(end_points[id_now][0]), int(end_points[id_now][1]))  # (row, col)

            path = tcod.path.path2d(cost=grid, start_points=[start_pt], end_points=[end_pt], cardinal=10, diagonal=14)
            print(f"Processing target {id_now}: distance = {dist_now:.2f}, waypoints = {path.shape[1] if path.ndim == 2 else 0}")
